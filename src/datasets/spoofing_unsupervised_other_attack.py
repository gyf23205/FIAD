...
from base.base_TimeNets import TimeNetsDataset
...
class SpoofingDatasetUnsupervisedOther(BaseADDataset):
    def __init__(self, root: str, dataset_name: str, n_known_outlier_classes: int = 0, ratio_known_normal: float = 0.0,
        ...
        X_train, X_test, y_train, y_test = train_test_split(signals, flags, test_size=test_ratio, random_state=random_state)
        ...
        X_test = np.load(os.path.join(path, 'data_sin_seq.npy'))
        y_test = np.load(os.path.join(path, 'labels_sin_seq.npy'))

        # Get training set
        self.train_set = TimeNetsDataset(X_train, y_train)
        ...
        self.val_set = TimeNetsDataset(X_val, y_val)

        #Get test set
        self.test_set = TimeNetsDataset(X_test, y_test)
