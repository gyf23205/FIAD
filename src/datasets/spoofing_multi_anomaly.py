...
class SpoofingMultiAnomalyPhysical(BaseADDataset):
    def __init__(self, root: str, dataset_name: str, n_known_outlier_classes: int = 0, ratio_known_normal: float = 0.0,
        ...
        signals1 = np.load(os.path.join(path,'data_multi_noise_batched.npy'))
        signals_next1 = np.load(os.path.join(path, 'next_multi_noise_batched.npy'))
        flags1 = np.load(os.path.join(path,'labels_multi_noise_batched.npy'))
        signals2 = np.load(os.path.join(path,'data_sin_batched.npy'))
        signals_next2 = np.load(os.path.join(path, 'next_sin_batched.npy'))
        flags2 = np.load(os.path.join(path,'labels_sin_batched.npy'))
        flags2[flags2==1] = 2 # Change label of sinusoid attack to 2
