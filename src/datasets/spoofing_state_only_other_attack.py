...
class SpoofingDatasetStateOnlyOther(BaseADDataset):
    def __init__(self, root: str, dataset_name: str, n_known_outlier_classes: int = 0, ratio_known_normal: float = 0.0,
        ...
        signals = np.load(os.path.join(path,'data_multi_noise_state_only_batched.npy'))
        signals_next = np.load(os.path.join(path, 'next_multi_noise_state_only_batched.npy'))
        flags = np.load(os.path.join(path,'labels_multi_noise_state_only_batched.npy'))
        idx_norm = flags==0
        idx_out = flags==1

         # The model only learn the dynamics of the system during training
        X_train_norm, _, y_train_norm, y_test_norm, next_train_norm, _ = train_test_split(signals[idx_norm], flags[idx_norm], signals_next[idx_norm],
                                                                        test_size=test_ratio, random_state=random_state)
                                                                                
        X_train_out, _, y_train_out, y_test_out, next_train_out, _ = train_test_split(signals[idx_out], flags[idx_out], signals_next[idx_out],
                                                                            test_size=test_ratio, random_state=random_state)

        X_train = np.concatenate([X_train_norm, X_train_out])
        y_train = np.concatenate([y_train_norm, y_train_out])
        X_test = np.load(os.path.join(path, 'data_sin_batched.npy'))
        y_test = np.load(os.path.join(path, 'labels_sin_batched.npy'))
