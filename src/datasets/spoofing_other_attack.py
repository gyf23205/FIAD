...
class SpoofingDatasetOther(BaseADDataset):
    def __init__(self, root: str, dataset_name: str, n_known_outlier_classes: int = 0, ratio_known_normal: float = 0.0,
        ...
        X_train_norm, _, y_train_norm, _, next_train_norm, _ = train_test_split(signals[idx_norm], flags[idx_norm], signals_next[idx_norm],
                                                                        test_size=test_ratio, random_state=random_state)
                                                                                
        X_train_out, _, y_train_out, _, next_train_out, _ = train_test_split(signals[idx_out], flags[idx_out], signals_next[idx_out],
                                                                            test_size=test_ratio, random_state=random_state)
        ...
        logger.info(f'n sample in test: Normal: {len(y_test[y_test==0])}, out: {len(y_test[y_test==0])}')
